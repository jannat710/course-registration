...
console.log(selectedCourse);
    return (
        <div className='bg-white w-64  p-3'>
            ...
            <h1 className='text-lg font-bold border-b-2 py-2'>Course Name :{selectedCourse.length}</h1>
            {
                selectedCourse.map(course =>(
                    <li key={course.id} className='list-decimal list-inside text-[#1c1b1b99] py-2'>{course.title}</li>
                ))
            }
            <h2 className='border-b-2 text-base font-medium py-2'>Total Credit Hour :{totalHour}</h2>
        </div>
    );
};
